...
function mobileScrollingAnimation(){
    ...
    downloadBtnContainer.style = "display: flex; align-items: center; flex-direction: column";
    ...
    if(windowY > 250){
        uniqueValuePropositionDiv.style.opacity = 1 - ((windowY - 250 ) / vidBackground.offsetHeight);
    ...
    if(1 - ((windowY - 250 )/ vidBackground.offsetHeight) <= 0) uniqueValuePropositionDiv.display = "none";
...
function computerScrollingAnimation(){
    ...
    if(windowY >= getPageYOffesetFromAngle(72)){
        if(1 - ((windowY - getPageYOffesetFromAngle(STOP_ANGLE))  / getPageYOffesetFromAngle(STOP_ANGLE)) <= 0) uniqueValuePropositionDiv.style.display = "none";
        else uniqueValuePropositionDiv.style.display = "block";
    
        uniqueValuePropositionDiv.style.opacity = 1 - ((windowY - getPageYOffesetFromAngle(STOP_ANGLE))  / getPageYOffesetFromAngle(STOP_ANGLE));
...
function downloadContainerMarginTop(offset){
    downloadBtnContainer.style.marginTop = `${windowY - vidBackground.offsetHeight + offset }px`;
